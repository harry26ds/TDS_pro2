...
def get_output_dir(dataset_name):
    # Create a directory based on the dataset name (without file extension)
    directory_name = dataset_name.split('.')[0]  # Removes extension (e.g., "dataset.csv" -> "dataset")
    if not os.path.exists(directory_name):
        os.makedirs(directory_name)  # Create directory if not exists
    return directory_name



def save_markdown(summary, insights, story=None, dataset_name="dataset.csv"):
    output_dir = get_output_dir(dataset_name)

    # Overwrite or create new README.md
    with open(f"{output_dir}/README.md", "w") as md_file:
